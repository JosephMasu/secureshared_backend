...
pub struct ErrorResponce{
    pub status: String,
    pub message: String,
}

impl fmt::Display for ErrorResponce {
...
pub struct HttpError{
 ...
 impl HttpError {
    ...
    pub fn unique_contraint_violation(message: impl Into<String>) -> Self {
        HttpError{
            message: message.into(),
    ...
    pub fn into_http_responce(self) -> axum::response::Response{
        let json_response = Json(ErrorResponce{
            status: "fail".to_string(),
            message: self.message,
        
        });
        (self.status, json_response).into_response().into()
    }
}
