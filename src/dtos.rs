...
pub struct RegisterUserDto {
    ...
    pub email: String,
    #[validate(
        length(min = 6, message = "Password is requiredand must be at least 6 characters")
    )]
    pub password: String,
...
pub struct UserPasswordUpdateDto {
    #[validate(
        length(min = 6, message = "New password and must be at least 6 characters")
    )]
    pub new_password: String,

    #[validate(
        length(min = 6, message = "New password and confirm must be at least 6 characters"),
...
fn validate_expiration_date(expiration_date: &String) -> Result<(), ValidationError> {
    if expiration_date.is_empty() {
        return Err(ValidationError::new("expiration_date_required"));
    }
